...
handleSimpleNode p trace nodeTracers nc onKernel = do
 ...
 where
  ...
  createTracers
    ...
    -> IO ()
  createTracers ncf@NodeConfiguration{ncNodeIPv4Addr, ncNodeIPv6Addr, ncValidateDB}
                tr tracer cfg = do
         ...
         traceWith tracer $ unlines
           [ ""
           , "**************************************"
           , "Host node addresses: " <> show (catMaybes [ nodeIPv4ToIPAddress <$> ncNodeIPv4Addr
                                                        , nodeIPv6ToIPAddress <$> ncNodeIPv6Addr
                                                        ])
