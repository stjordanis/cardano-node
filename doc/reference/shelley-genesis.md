...
$ cardano-cli shelley genesis initial-addr \
    --verification-key-file example/utxo-keys/utxo1.vkey \
    --testnet-magic 42
  --TODO: Currently outputs bech32 while addresses are base16 encoded in shelley genesis
...
`networkMagic` from the `genesis.json`. In the default we generated, that was --TODO: What?
...
So we have to specify `--testnet-magic 1097911063`, otherwise it defaults to mainnet --TODO: This is what I thought
...
time in the command.
--TODO: Double check this command
```
...
`genesis.json` of course. So let's try it. Obviously you will have to adjust --TODO: I think the address needs to be bech32 encoded
this command to use the right address(es) from your `genesis.json`.
--TODO: Verify this command & output
```
$ CARDANO_NODE_SOCKET_PATH=example/node1/node.sock \
    cardano-cli shelley query filtered-utxo \
    --testnet-magic 1097911063 \
